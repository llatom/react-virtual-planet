...
const roof = new THREE.Mesh(new THREE.ConeBufferGeometry(3.5, roofHeight, 4), new THREE.MeshStandardMaterial({ color: '#b35f45' }))
...
for (let i = 0; i < graveCount; i++) {
  const r = ky.randomNumberInRange(3, 9)
  const angle = ky.deg2rad(ky.randomNumberInRange(0, 360))
  const x = Math.cos(angle) * r
  const z = Math.sin(angle) * r

  const grave = new THREE.Mesh(graveGeometry, graveMaterial)
  grave.position.set(x, 0.3, z)
  grave.rotation.y = ky.randomNumberInRange(-0.5, 0.5) * 0.4
  grave.rotation.z = ky.randomNumberInRange(-0.5, 0.5) * 0.4
  graves.add(grave)
}
